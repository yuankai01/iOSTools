...
- (id)initWithFrame:(CGRect)frame tableViewOrientation:(EasyTableViewOrientation)easyTableViewOrientation {
    if (self = [super initWithFrame:frame]) {
		_numItems			= 0;
		_cellWidthOrHeight	= 0;
		
		[self createTableWithOrientation:easyTableViewOrientation];
	}
    return self;
}
...
- (void)createTableWithOrientation:(EasyTableViewOrientation)orientation {
	...
	if (orientation == EasyTableViewOrientationHorizontal) {
        // 横向的tableView 需要绕中心点旋转90度。一下计算旋转前的坐标。tableView 和 self 的中心点一致。
...
- (void)scrollToCellAtIndexPath:(NSIndexPath *)indexPath animated:(BOOL)animated {
    // 取消滑动
    if (self.tableView.contentSize.width <= self.tableView.bounds.size.width &&
        self.tableView.contentSize.height <= self.tableView.bounds.size.height) {
        return;
    }
    
    
    CGFloat cellWidthOrHeight = [self tableView:self.tableView heightForRowAtIndexPath:indexPath];
    
    UIView *headerView = [self tableView:self.tableView viewForHeaderInSection:indexPath.section];
    
    float offDistance;
    if (_orientation == EasyTableViewOrientationHorizontal) {
		offDistance = self.bounds.size.width/2 - cellWidthOrHeight/2 - headerView.bounds.size.width;
	}
	else {
		offDistance = self.bounds.size.height/2 - cellWidthOrHeight/2 - headerView.bounds.size.height;
	}
    
    
	CGPoint defaultOffset	= CGPointMake(0, indexPath.row  *cellWidthOrHeight - offDistance);
...
#pragma mark - // tableview 的 scrollView 的delegate
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    int index = scrollView.contentOffset.y/scrollView.bounds.size.height;
    //ILog(@" \n scrollViewDidEndDecelerating \n %@ \n %i \n  ",scrollView,index);
    if ([delegate respondsToSelector:@selector(easyTableView:scrollDidEndDeceleratingAtIndex:)]) {
        [delegate easyTableView:self scrollDidEndDeceleratingAtIndex:index];
    }
