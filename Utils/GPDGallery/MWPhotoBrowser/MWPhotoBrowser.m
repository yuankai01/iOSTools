...
- (void)tilePages {
	...
	for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
		if (![self isDisplayingPageForIndex:index]) {
            ...
            if(!bottomLabel) {
                bottomLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, _pagingScrollView.frame.size.width - 60, _pagingScrollView.frame.size.height, 40)];
                [_pagingScrollView.superview addSubview:bottomLabel];
            }
            bottomLabel.backgroundColor = [UIColor darkGrayColor];
            MWPhoto *photo = [_photos objectAtIndex:index];
            bottomLabel.text = photo.description;
