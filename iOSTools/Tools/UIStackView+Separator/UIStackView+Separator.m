...
@property (strong, nonatomic) NSArray *disSeparatorIndex;
...
- (void)makeSeparators {
    ...
    [strongStackView.arrangedSubviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        //哪个inex不需要分割线
        for (id tempView in self.disSeparatorIndex) {
            if ([obj isEqual:tempView]) {
                return;
            }
        }
...
- (void)setDisSeparatorIndex:(NSArray *)disSeparatorIndex
{
    [self _separatorHelper].disSeparatorIndex = disSeparatorIndex;
    [[self _separatorHelper] makeSeparators];
}
...
- (NSArray *)disSeparatorIndex
{
    return [self _separatorHelper].disSeparatorIndex;
}
